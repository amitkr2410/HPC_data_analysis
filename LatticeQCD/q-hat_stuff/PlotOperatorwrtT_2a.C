...
void PlotOperatorwrtT_2a()
{
  ...
  TGraphErrors *gbeta, *g, *Gbeta, *G, *g2, *G2, *g3, *G3,  *g4, *G4;
  TGraphErrors *hbeta, *h, *H, *h2, *H2, *h3, *H3, *h4, *H4;
  int PowersOfU0Clover=4, PowersOfU0FourLinks=4; int Nt, Ns;
  
  Nt=4; Ns=16;
  sprintf(FileName,"%sDataPloopNt%d_Ns%d.txt", QCDTypeS.c_str(), Nt, Ns);
  gbeta = new TGraphErrors(FileName,"%lf %*lf %*lf %lf");
  ...
  g  = Divideg0Square(g,  gbeta, QCDTypeS);
  g2 = Divideg0Square(g2, gbeta, QCDTypeS);
  g3 = Divideg0Square(g3, gbeta, QCDTypeS);
  g4 = Divideg0Square(g4, gbeta, QCDTypeS);
  ...
  Nt=8; Ns=32;
  sprintf(FileName,"%sDataPloopNt%d_Ns%d.txt", QCDTypeS.c_str(), Nt, Ns);
  hbeta = new TGraphErrors(FileName,"%lf %*lf %*lf %lf");
  sprintf(FileName,"Nt%d_%s.txt",Nt, QCDTypeS.c_str());
  ...
  h  = Divideg0Square(h,  hbeta, QCDTypeS);
  //h2 = Divideg0Square(h2, hbeta, QCDTypeS);  //
  h3 = Divideg0Square(h3, hbeta, QCDTypeS);
  h4 = Divideg0Square(h4, hbeta, QCDTypeS);
  ...
  for(int i=0;i<N;i++)
    {
      ...
      y[i]  = Y[i]*StrongCouplingOneLoop(QCDTypeS,X[i])/g0Square ;
      ey[i] = EY[i]*StrongCouplingOneLoop(QCDTypeS,X[i])/g0Square ;
    }
  TGraphErrors *gr = new TGraphErrors(N,x,y,ex,ey);
  return gr;
}
