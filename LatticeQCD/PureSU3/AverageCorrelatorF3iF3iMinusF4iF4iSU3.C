...
double LatticeCalculation::TraceFieldStrengthSquare(int n1,int n2,int n3, int n4)
{
  ...
  TComplex PlaquetteMatrix[3][3]={0.0},FieldStrengthMatrix[3][3]={0.0},U1,U2,U3,U4,TraceFSquare;
...
void AverageCorrelatorF3iF3iMinusF4iF4iSU3(int nt,int nx,double Beta)
{ int StartTime = time(NULL);
  ...
  LC.TotalConfiguration = 200; //Number of gauge field configuartion over which average of (F3iF3i-F4iF4i) is computed         
...
//Labeling each link with its 4 space-time coordinate and a direction
  ...
  for(TemperatureLoop = 0; TemperatureLoop<LC.TotalTemperatureRun; TemperatureLoop++)  
      ...
      for(Int_t Iter1=0; Iter1<LC.Run; Iter1++)
	{
	  ...
	  cout<<"Tr(F3i^2)/(1.0*N31*T^4) - Tr(F4i^2)/(1.0*N41*T^4)=  "<<(TraceF31F31PerPlaquette[Iter]+TraceF32F32PerPlaquette[Iter]-TraceF41F41PerPlaquette[Iter]-TraceF42F42PerPlaquette[Iter])/(1.0*pow(LC.temperature,4.0))<<endl;
         cout<<"ScaledAverageFmunu^2 So far = "<<SumFieldStrengthCorrelationPerPlaquette/(1.0*pow(LC.temperature,4.0)*(Iter+1))<<endl;
	 cout<<"So far a^4*Sigma(F3iF3i-F4iF4i)/1.0 = "<<pow(LC.LatticeSpacing,4.0)*SumFieldStrengthCorrelationPerPlaquette/(1.0*(Iter+1))<<endl;
      ...
      for(int i=0; i<LC.TotalConfiguration; i++)
	{
      ...
      ScaledAverageFieldStrengthCorrelation[TemperatureLoop] = (AverageTraceF31F31PerPlaquette  + AverageTraceF32F32PerPlaquette - AverageTraceF41F41PerPlaquette - AverageTraceF42F42PerPlaquette  )/(1.0*pow(LC.temperature,4.0));

      BetaAverageCorrelator[TemperatureLoop] = pow(LC.LatticeSpacing,4.0)*(AverageTraceF31F31PerPlaquette  + AverageTraceF32F32PerPlaquette - AverageTraceF41F41PerPlaquette - AverageTraceF42F42PerPlaquette)/1.0;
      
      ScaledAvgFSCorrelationError[TemperatureLoop] =  TMath::Sqrt( pow(ErrorF31F31,2.0) + pow(ErrorF32F32,2.0) + pow(ErrorF41F41,2.0) + pow(ErrorF42F42,2.0) )/(1.0*pow(LC.temperature,4.0));
      
      BetaAvgCorrelatorError[TemperatureLoop] = pow(LC.LatticeSpacing,4.0)*TMath::Sqrt( pow(ErrorF31F31,2.0) + pow(ErrorF32F32,2.0) + pow(ErrorF41F41,2.0) + pow(ErrorF42F42,2.0) )/1.0;
