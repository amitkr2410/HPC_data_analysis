...
void AnalyzeNtNs(int Nt,int Ns,int QCDType,int FmunuType, int NNLOFlag,  string File0, string File00, string File1,  string File11, string File12, string File13, string File14, string File21, string File22); 
...
void StatisticalAverageAll()
{
  ...
  AnalyzeNtNs(Nt, Ns, QCDType,FmunuType, NNLOFlag, Output0, Output00, Output1,  Output11, Output12, Output13, Output14,  Output21, Output22);
...
void AnalyzeNtNs(int Nt, int Ns, int QCDType, int FmunuType, int NNLOFlag, string OutputFile0, string OutputFile00, string OutputFile1, string OutputFile11, string OutputFile12, string OutputFile13, string OutputFile14,string OutputFile21, string OutputFile22)
{
  CustomGlobal();
	...
	{ 
	  for(int j=0; j<4;j++)           // LO or NLO or NNLO or NNNNLO Fmunu
	    { if(j==0) {LOorNLO="LO_";}	    
	      ...
	      cout<<"\n F3i"<<j<<"F3i.Re (Nt,Ns)="<<NtTemp<<","<<Ns<<": Beta="<<Beta[i]<<endl;
	 ...
	 if(QCDType>0 && k==1 && j==1 && Nt >=10) //NLO, Vacuum
 ...
 stad=SubtractTGraphErrors(DivideTGraphErrors(g,1,gU0,2),DivideTGraphErrors(gv,1,gU02,2)); stad2=SubtractTGraphErrors(DivideTGraphErrors(g2,1,gU0,2),DivideTGraphErrors(gv2,1,gU02,2)); stad3=SubtractTGraphErrors(DivideTGraphErrors(g3,1,gU0,2),DivideTGraphErrors(gv3,1,gU02,2));stad4=SubtractTGraphErrors(DivideTGraphErrors(g4,1,gU0,2),DivideTGraphErrors(gv4,1,gU02,2)); stad5=SubtractTGraphErrors(DivideTGraphErrors(g5,1,gU0,2),DivideTGraphErrors(gv5,1,gU02,2)); stad6=SubtractTGraphErrors(DivideTGraphErrors(g6,1,gU0,2),DivideTGraphErrors(gv6,1,gU02,2)); stad7=SubtractTGraphErrors(DivideTGraphErrors(g7,1,gU0,2),DivideTGraphErrors(gv7,1,gU02,2)); stad8=SubtractTGraphErrors(DivideTGraphErrors(g8,1,gU0,2),DivideTGraphErrors(gv8,1,gU02,2));//Real-part LO1 LO2 NLO1 NLO2 NNLO1 NNLO2 NNNNLO1 NNNNLO2
 Stad=SubtractTGraphErrors(DivideTGraphErrors(G,1,gU0,2),DivideTGraphErrors(Gv,1,gU02,2)); Stad2=SubtractTGraphErrors(DivideTGraphErrors(G2,1,gU0,2),DivideTGraphErrors(Gv2,1,gU02,2)); Stad3=SubtractTGraphErrors(DivideTGraphErrors(G3,1,gU0,2),DivideTGraphErrors(Gv3,1,gU02,2));Stad4=SubtractTGraphErrors(DivideTGraphErrors(G4,1,gU0,2),DivideTGraphErrors(Gv4,1,gU02,2)); Stad5=SubtractTGraphErrors(DivideTGraphErrors(G5,1,gU0,2),DivideTGraphErrors(Gv5,1,gU02,2)); Stad6=SubtractTGraphErrors(DivideTGraphErrors(G6,1,gU0,2),DivideTGraphErrors(Gv6,1,gU02,2)); Stad7=SubtractTGraphErrors(DivideTGraphErrors(G7,1,gU0,2),DivideTGraphErrors(Gv7,1,gU02,2)); Stad8=SubtractTGraphErrors(DivideTGraphErrors(G8,1,gU0,2),DivideTGraphErrors(Gv8,1,gU02,2)); //Im-part   LO1 LO2 NLO1 NLO2 NNLO1 NNLO2 NNNNLO1 NNNNLO2  
 ...
 c1->cd(1);
 Custom(g,2); Custom(g2,1); Custom(g3,28); Custom(g4,8); Custom(g5,4); Custom(g6,6); Custom(g7,41); Custom(g8,46);
 ...
 c1->cd(3);
 Custom(gv,2); Custom(gv2,1); Custom(gv3,28); Custom(gv4,8); Custom(gv5,4); Custom(gv6,6); Custom(gv7,41); Custom(gv8,46);
...
//MinMax=FindMinMax(Gv,Gv2,Gv3,Gv4); Diff = (MinMax[1]-MinMax[0])/6.0;
 ...
 c2->cd(1);
 Custom(s,2); Custom(s2,1); Custom(s3,28); Custom(s4,8); Custom(s5,4); Custom(s6,6); Custom(s7,41); Custom(s8,46);
 s->SetMarkerStyle(22);  s2->SetMarkerStyle(23);  s3->SetMarkerStyle(25);  s4->SetMarkerStyle(3); s5->SetMarkerStyle(35); s6->SetMarkerStyle(37); s7->SetMarkerStyle(24); s8->SetMarkerStyle(27);
 s->Draw("AL p");s2->Draw("Lp same ");s3->Draw("Lp same "); s4->Draw("Lp same "); s5->Draw("Lp same "); s6->Draw("Lp same "); s7->Draw("Lp same "); s8->Draw("Lp same ");
 MinMax=FindMinMax6(s,s2,s3,s4,s5,s6); Diff = (MinMax[1]-MinMax[0])/6.0;
 s->GetYaxis()->SetRangeUser(MinMax[0]-Diff,MinMax[1]+Diff);
 s->GetYaxis()->SetTitle("(Thermal-Vacuum): Real<#it{O}>"); s->GetXaxis()->SetTitle("#beta");
 gPad->SetMargin(0.2,0.02,0.2,0.1); s->GetYaxis()->SetTitleOffset(1.6);

 c2->cd(2);
 Custom(S,2); Custom(S2,1); Custom(S3,28); Custom(S4,8); Custom(S5,4); Custom(S6,6); Custom(S7,41); Custom(S8,46);
 S->SetMarkerStyle(22);  S2->SetMarkerStyle(23);  S3->SetMarkerStyle(25); S4->SetMarkerStyle(3);S5->SetMarkerStyle(35); S6->SetMarkerStyle(37); S7->SetMarkerStyle(24); S8->SetMarkerStyle(27);
 S->Draw("ALp ");S2->Draw(" Lp same"); S3->Draw("Lp same "); S4->Draw("Lp same "); S5->Draw("Lp same "); S6->Draw("Lp same "); S7->Draw(" Lp same"); S8->Draw(" Lp same");
//MinMax=FindMinMax(S,S2,S3,S4); Diff = (MinMax[1]-MinMax[0])/6.0;
 S->GetYaxis()->SetRangeUser(MinMax[0]-Diff,MinMax[1]+6*Diff);
 S->GetYaxis()->SetTitle("(Thermal-Vacuum): Imag<#it{O}>");S->GetXaxis()->SetTitle("#beta");
 gPad->SetMargin(0.16,0.02,0.2,0.1);S->GetYaxis()->SetTitleOffset(1.6);
 TLEGEND4(0.2,0.5,0.88,0.88, S,"#it{g^{2}a^{4}(F3iF3i - F4iF4i)}","elp", S2,"#it{g^{2}a^{4}(F3iF4i + F4iF3i)}","elp", S3,"#it{g^{2}a^{5}(F3iDzF3i - F4iDzF4i)}","elp", S4,"#it{g^{2}a^{5}(F3iDzF4i + F4iDzF3i)}","elp", 0.05);

 c2->cd(3);
 Custom(ct,2); Custom(ct2,1); Custom(ct3,28); Custom(ct4,8); Custom(ct5,4); Custom(ct6,6); Custom(ct7,41); Custom(ct8,46);
 ...
 TLEGEND4(0.2,0.5,0.88,0.88, CT,"#it{<F3iF3i - F4iF4i>/T^{4}}","elp", CT2,"#it{<F3iF4i + F4iF3i>/T^{4}}","elp", CT3,"#it{<F3iDzF3i - F4iDzF4i>/(T^{4}q^{-})}","elp", CT4,"#it{<F3iDzF4i + F4iDzF3i>/(T^{4}q^{-})}","elp", 0.05);
...
TGraphErrors * ConvertToT(TGraphErrors *g , int Nt, int QCDType, double Qminus, string LOorNLO)
{
  ...
  double Beta, T, a0, g0Square, Rbeta, PreFactor=0;
  for(int i=0; i<N; i++)
    {
      Beta = X[i];
      ...
      if(LOorNLO=="LO")  { PreFactor=0.5/(g0Square*pow(a0,4)); }
      if(LOorNLO=="NLO") { PreFactor=0.5*sqrt(2)/(g0Square*pow(a0,5)*Qminus); }
      if(LOorNLO=="NNLO") { PreFactor=-1/(g0Square*pow(a0,6)*Qminus*Qminus); }
      if(LOorNLO=="NNNNLO") { PreFactor=2/(g0Square*pow(a0,8)*pow(Qminus,4)); }

      cout<<LOorNLO<<"\t,Nt="<<Nt<<",Beta="<<Beta<<",T="<<T<<endl;
      cout<<"ConvertToT: PreFactor="<<PreFactor<<",Rbeta="<<Rbeta<<endl;
      y[i]  = Y[i]*PreFactor*Rbeta/pow(T,4);
      ey[i] = EY[i]*PreFactor*Rbeta/pow(T,4);
    }
...
void QhatRe(int Nt, int QCDType, double *Qminus, string File1Output,  string File3Output)
{
  ...
  double T, a0, Beta, g0Square, Factor; 
  ...
  for(int i=0;i<N;i++)
    {
      ...
      T = LatticeSpacing( Nt, QCDType, Beta);
      a0= 1.0/(Nt*T);
      Factor = 1/((g0Square*pow(a0,4.0)));
      Y[i]=PreFactor*Factor*( (0.5*Y1[i]) + (0.5*sqrt(2.0)*(-Y2[i])/(Qminus[0]*a0)) + (-Y3[i]/pow(Qminus[0]*a0,2)) + (2*Y4[i]/pow(Qminus[0]*a0,4.0)) )/pow(T,4.0);
      Ey[i]=PreFactor*Factor*sqrt(pow(0.5*Ey1[i],2.0)  + pow(0.5*sqrt(2.0)*Ey2[i]/(Qminus[0]*a0),2.0) + pow(Ey3[i]/pow(Qminus[0]*a0,2.0),2.0)      +  pow( 2*Ey4[i]/pow(Qminus[0]*a0,4.0),2.0)  )/pow(T,4.0);
      Ex[i]=0;
      X[i] = T;
