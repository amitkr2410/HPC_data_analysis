	...
	for(int FileIndex=0; FileIndex<NumberOfFiles; FileIndex++)
	  {
	    ...
	    sprintf(HadronFile,"%s",argv[1]);  // name of the input file  
	    sprintf(HardCrossSectionFile,"%s",argv[1]);
		...
		if( SN==SNadvance && fjInputs.size()>0 )  // construct jet for the previous event
		  {
		...
		if(SN==SNadvance)
		  {
		    ...
		    Events[k]++; EventsPerFile++;SNadvance++;
	    ...
	    HardCrossSection[k]=1.0;HardCrossSectionError[k]=0.0;
