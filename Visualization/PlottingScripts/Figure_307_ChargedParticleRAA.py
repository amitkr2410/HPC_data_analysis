...
files_aa_theory = ['../Liquefier_BroadeningEcut8T/ColorlessSingleHadronYield_PbPb_Jetscape_CMS_5020GeV_EtaMax_1p0_FS_ChargedHadron_Cent_0-10_MatterLBT_withRecoil_Liqu_3p2T_Broad_8TType5_alphas_0p35_Q0_2GeV_BGS1.txt']
                   
#$\hat{q}=\hat{q}^{fix}_{HTL}f(Q^{2}), \mathrm{JS(MATTER+LBT) }$
label_theory = ['$\mathrm{JS(MATTER+LBT)}$']
...
OutputFilename='../Plots/ChargedParticle_RAA_Type5_Liq3p2T_Broad_8T_5020GeV_CMS_0-10_eta1p0.pdf'
x_min =8.0
x_max =450
y_min = 0
y_max = 1.7
...
axs.legend(handles,labels,ncol=1,loc='center left',edgecolor='none', frameon=True, facecolor='none', handletextpad=0.4, handleheight=1.8, labelspacing=0.2, bbox_to_anchor=(0, 0.72), borderaxespad=0.8, handlelength=1.6, fontsize=12)


## Text
axs.text(x_min+1, y_max-0.3, '$\mathrm{PbPb~(0\\text{-}10\%),~\sqrt{\it{s}_{NN}}=5.02~TeV}$\n'+ '$|\eta|<1.0$' , horizontalalignment='left', verticalalignment='bottom', fontsize=12)
axs.text(x_max-80, y_min+0.04,'$\\hat{q}=\\hat{q}^{run}_{HTL}f(Q^{2})$\n $\\alpha^{fix}_{s}\!\!=\!\!0.35,Q_{sw}\!\!=\!\!\mathrm{2~GeV}$ \n $~\mathrm{Parton~Ecut=3.2T},~\mathrm{Correlated~Broadening~Ecut=8T}$',horizontalalignment='right', verticalalignment='bottom',fontsize=12)
## Generate PDF File
plt.tight_layout()
plt.savefig('temp2')
## Add Logos
