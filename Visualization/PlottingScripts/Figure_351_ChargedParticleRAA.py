...
##--
file_pp_theory = '../2760GeV/ColorlessSingleHadronYield_pp_Jetscape_CMS_2760GeV_EtaMax_1p0_FS_ChargedHadron_Cent_0-5_MATTER_ppPbPb_BGS0.txt'

files_aa_theory = ['../2760GeV/ColorlessSingleHadronYield_PbPb_Jetscape_CMS_2760GeV_EtaMax_1p0_FS_ChargedHadron_Cent_0-5_MatterLBT_withRecoil_Type5_alphas_0p3_Q0_2GeV_BGS1.txt']
                   

label_theory = [r'$\hat{q}=\hat{q}^{run}_{HTL}f(Q^{2}), \mathrm{JS(MATTER+LBT) }$']

files_data = ['../../ExperimentData_All/ExpData_CMS_2760_ChargedHadron_Raa_Eta1p0_Cent0-5.dat']

label_data = ['$\mathrm{CMS~[EPJ~C72,~1945~(2012)]}$']
OutputFilename='../Plots/ChargedParticle_RAA_2760GeV_CMS_0-5_eta1p0.pdf'
x_min =6.0
x_max =110
y_min = 0
y_max = 1.6
...
## Text
axs.text(x_min+4, y_max-0.3, '$\mathrm{PbPb~(0\\text{-}5\%),~\sqrt{\it{s}_{NN}}=2.76~TeV}$\n'+ '$|\eta|<1.0$' , horizontalalignment='left', verticalalignment='bottom')
axs.text(x_min+14, y_max-0.76,'$\mathrm{\\alpha^{fix}_{s}=0.3},~Q_{sw}=2~\mathrm{GeV}$',horizontalalignment='left', verticalalignment='bottom')
## Generate PDF File
plt.tight_layout()
plt.savefig('temp2')
## Add Logos
