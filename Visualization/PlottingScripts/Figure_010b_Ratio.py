...
color_theory = ['green','red','blue']
line_theory = ['dotted','solid','dashed']
line_width_theory = ['5','3','3']
...
##--
file_denom = '../200GeV/JetCrossSection_R_0p6_AntikT_pp_Pythia_Star_200GeV_EtaJetMin_0p0_EtaMax_1p0_FS_allHadron.txt'

files_theory = ['../ColoredHadronization/JetCrossSection_R_0p6_AntikT_pp_Jetscape_Star_200GeV_EtaJetMin_0p0_EtaMax_1p0_FS_allHadron.txt',
                '../200GeV/ColorlessJetCrossSection_R_0p6_AntikT_pp_Jetscape_Star_200GeV_EtaJetMin_0p0_EtaJetMax_1p0_FS_allHadron_pTTrackMin_0p0GeV.txt',
                '../200GeV/JetCrossSection_R_0p6_AntikT_pp_Pythia_Star_200GeV_EtaJetMin_0p0_EtaMax_1p0_FS_allHadron.txt']

label_theory = [r'$\mathrm{JETSCAPE~3.0~(Colored~Had.)}$', r'$\mathrm{JETSCAPE~3.0~ (Colorless~ Had.)}$',
                r'$\mathrm{PYTHIA~8}$']

files_data = ['../../ExperimentData_All/ExpData_STAR_200_JetCrosssection_ppbaseline_Rp6_eta1p0.dat']
#ATLAS PLB 790, 108 (2019)
label_data = ['$\mathrm{STAR~[PoS~DIS2015,~203~(2015)]}$']
OutputFilename='../Plots/RatioPlot_Colored_DJCS_200GeV_STAR_R0p6_eta1p0.pdf'
########################################################################################################################################################          
...
for i, file in enumerate(files_data):
  ...
  axs[0].errorbar(x, factor_data[i]*y, yerr=factor_data[i]*yerr, linestyle="none", color = color_data[i], marker=marker_data[i])
  # Shades for sys errors
  axs[0].fill_between(xstep, factor_data[i]*(ystep-ysysl), factor_data[i]*(ystep+ysysh), step='post', alpha=0.2, color = color_data[i])
############################################################################################################################################
## Plot Theory Lines
for i, file in enumerate(files_theory):
...
for i, file in enumerate(files_data):
  ...
  # Markers with error bars for stat errors
  axs[1].errorbar(x, factor_data[i]*y, yerr=factor_data[i]*yerr, linestyle="none", color = color_data[i], marker=marker_data[i])
  # Shades for sys errors
  axs[1].fill_between(xstep, factor_data[i]*(ystep-ysysl), factor_data[i]*(ystep+ysysh), step='post', alpha=0.2, color = color_data[i])
############################################################################################################################################
# Legends
handles = []
labels = []
...
axs.set_ylabel(r'$\mathrm{Ratio~to~PYTHIA~8}~\left[\frac{d^2 \sigma}{d\eta dp_{\mathrm{T}}^{\mathrm{jet}}}\right]$', fontsize=19)
...
for i, file in enumerate(files_theory):
  x, y, xerr, yerr, xstep, ystep = GetRatioTheoryToTheory(file,file_denom)
  # Steps
  axs.step(xstep, factor_theory[i]*ystep, where='post', color=color_theory[i],linestyle=line_theory[i],linewidth=line_width_theory[i])
  # Error bars for stat errors  
...
for i, file in enumerate(files_data):
  x, y, xerr, yerr, xstep, ystep, ysysl, ysysh = GetRatioExpToTheory(file,file_denom)
  # Markers with error bars for stat errors
  axs.errorbar(x, factor_data[i]*y, yerr=factor_data[i]*yerr, linestyle="none", color = color_data[i], marker=marker_data[i])
  # Shades for sys errors
  axs.fill_between(xstep, factor_data[i]*(ystep-ysysl), factor_data[i]*(ystep+ysysh), step='post', alpha=0.2, color = color_data[i])
...
for i, file in enumerate(files_theory):
  ...
  handles.append(tl[0]) 
  labels.append(label_theory[i])   
axs.legend(handles,labels,ncol=1,loc='lower left',edgecolor='none', frameon=True, facecolor='none', handletextpad=0.4, handleheight=1.6, labelspacing=0.05, bbox_to_anchor=(0.05, -0.02), borderaxespad=0.5, handlelength=1.6, fontsize=14)

## Text
axs.text(1.2*x_min,0.95*y_max, '$\mathrm{pp},~\sqrt{s}=200\mathrm{~GeV}$\n'+r'$\mathrm{anti\text{-}}k_\mathrm{T}$, $R\!=\!0.6$'+'\n'+'$|\eta_\mathrm{jet}|<1.0$', horizontalalignment='left', verticalalignment='top')

## Generate PDF File
plt.tight_layout()
plt.savefig('temp2')
## Add Logos
add_jetscape_logo('temp2',310,230,80,'_logo')
os.remove('temp.pdf')
