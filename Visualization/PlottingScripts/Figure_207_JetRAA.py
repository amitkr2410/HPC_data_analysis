...
files_aa_theory = [ '../Liquefier_BroadeningEcut8T/ColorlessJetCrossSection_R_0p4_AntikT_PbPb_Jetscape_Alice_5020GeV_EtaJetMin_0p0_EtaJetMax_0p3_FS_allHadron_Cent_0-10_MatterLBT_withRecoil_Lique3p2T_Br8T_Type5_alphas_0p3_Q0_2GeV_BGS1_pTTrackMin_0p0GeV.txt']
                   
label_theory = [r'$\mathrm{JS(MATTER+LBT)}$']
...
OutputFilename='../Plots/Jet_RAA_5020GeV_Type5_Lique3p2T_Broad_8T_ALICE_R0p4_eta0p3.pdf'
x_min =50.0
x_max =150
y_min = -0.2
y_max = 1.0
...
## Text
axs.text(x_max-2, y_min+0.16, '$\mathrm{PbPb~(0\\text{-}10\%),~\sqrt{\it{s}_{NN}}=5.02~TeV}$\n'+ '$\mathrm{anti}\\text{-}k_{\mathrm{T}},~\!\!R\!\!=\!\!0.4,~|\\eta_{\mathrm{jet}}|<0.3,p^{\mathrm{lead}}_{\mathrm{T,ch}}>7\mathrm{GeV}$' , horizontalalignment='right', verticalalignment='bottom',fontsize=12)
axs.text(x_max-2, y_min+0.03,' $\\hat{q}\!\!=\!\!\\hat{q}^{run}_{HTL}f(Q^{2}),\\alpha^{fix}_{s}\!\!=\!\!0.35,~ \! Q_{sw}\!\!=\!\!2~\!\mathrm{GeV}$ \n $~\mathrm{Parton~Ecut=3.2T},~\mathrm{Correlated~Broadening~Ecut=8T}$',horizontalalignment='right', verticalalignment='bottom',fontsize=12)
## Generate PDF File
plt.tight_layout()
plt.savefig('temp2')
## Add Logos
add_jetscape_logo('temp2',84,82,80,'_logo')
os.remove('temp.pdf')
