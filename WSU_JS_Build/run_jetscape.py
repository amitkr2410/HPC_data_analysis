...
def Main():
    ...
    JETSCAPEDIR="/wsu/tmp/AmitAAPaper/HPC_data_analysis/WSU_JS_Build/JETSCAPE"
    TestOutDIR="/wsu/tmp/AmitAAPaper/HPC_data_analysis/WSU_JS_Build/RawFile"
    FinalOutputDIR="/wsu/tmp/AmitAAPaper/HPC_data_analysis/WSU_JS_Build/FinalData" 
    CodeDIR="/wsu/tmp/AmitAAPaper/HPC_data_analysis/WSU_JS_Build"
    Exec=CodeDIR +"/RUN.sh"
    UserXML="/wsu/tmp/AmitAAPaper/HPC_data_analysis/WSU_JS_Build/jetscape_user_music_grid.xml"
    #Number of pthat-bins is 23
    #Number of subruns for each pthat-bins are 10
    NumberOfpTHatBins=2     #23
    NumberOfSubRun=2
    for i in range(0,NumberOfpTHatBins):
        for run in range(0,NumberOfSubRun):        
            ...
            Command = Command + " "+ JETSCAPEDIR +" "+  TestOutDIR + " "+ FinalOutputDIR + " "+ CodeDIR + " " + BuildDIR
            print(Command)
