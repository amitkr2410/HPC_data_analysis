...
int main(int argc, char **argv){
  double PI=3.1415926;
    ...
    // Number of charged hadron vs pT of the hadron
    TH1D *HistTempSingleHadron[NpTHardBin]; //Count vs pT of hadron for single-hadron
    TH1D *HistTempSingleHadronPositive[NpTHardBin]; //Positive particles 
    TH1D *HistTempSingleHadronNegative[NpTHardBin]; //Negative particles
    char HistName[10000];
    for (int k = 0; k<NpTHardBin; ++k)
      {
	sprintf(HistName,"CountVspTSingleHadronSpectrumBin%i_%i",pTHatMin[k],pTHatMax[k]);
	HistTempSingleHadron[k] = new TH1D(HistName, "Single Hadron Spectrum pT", NpTSingleHadronBin, SingleHadronpTBin); //CountVspT
	
	sprintf(HistName,"CountVspTSingleHadronSpectrumBin%i_%iPositive",pTHatMin[k],pTHatMax[k]);
	HistTempSingleHadronPositive[k] = new TH1D(HistName, "Single Hadron Spectrum pT", NpTSingleHadronBin, SingleHadronpTBin);

	sprintf(HistName,"CountVspTSingleHadronSpectrumBin%i_%iNegative",pTHatMin[k],pTHatMax[k]);
	HistTempSingleHadronNegative[k] = new TH1D(HistName, "Single Hadron Spectrum pT", NpTSingleHadronBin, SingleHadronpTBin);
      }
    ...
    // For loop to open different pTHat bin files, in this default example, only one file for each pTHatBin
	...
	Events[k]=0;
        ...
        HistTempSingleHadron[k]->Reset();
	HistTempSingleHadronPositive[k]->Reset();
	HistTempSingleHadronNegative[k]->Reset();
		...
		if(MyString.compare(EventLabel)!=0)
		  { //myfile >> PID >> pStat >> E >> Px >> Py >> Pz ;
			...
			if(ChargedFlag==1 && pythia.particleData.charge( PID )!=0 && fabs(PID) > 100)
			  {
			    HistTempSingleHadronPositive[k]->Fill(PT); // fill pT distribution of single hadron charged
			  }

			if(ChargedFlag==0)
                          {
                            HistTempSingleHadronPositive[k]->Fill(PT); // fill pT distribution of single hadron charged and neutral 
			  }
		    ...
		    if(pStat<0 && fabs(Eta) < SingleHadronEtaCut) 
		      {
			//cout<<"PID "<<PID<<"\t charge = "<<pythia.particleData.charge( PID)<<endl;
			HistTempSingleHadronNegative[k]->Fill(PT); // fill pT distribution of single hadron       
			SumE= SumE - E; SumPx = SumPx - Px; SumPy= SumPy - Py; SumPz = SumPz - Pz;
		      }
	...
	if(BGS==1)
	  {
	    HistTempSingleHadron[k]->Add(HistTempSingleHadronPositive[k], HistTempSingleHadronNegative[k],1.0,-1.0);
	  }
	else
	  {
	    HistTempSingleHadron[k]->Add(HistTempSingleHadronPositive[k], HistTempSingleHadronNegative[k],1.0,0.0);
	  }
	//For single Hadron spectrum
	for(int j=0;j<NpTSingleHadronBin;j++)
	  {
	    dNdpTCountSingleHadron[k][j]= HistTempSingleHadron[k]->GetBinContent(j+1);
	...
	//Write stuff into files
        #pragma omp ordered
	   {
	     ...
	     cout<<"For bin k="<<k<<",pTHat("<< pTHatMin[k]<<","<<pTHatMax[k] <<")GeV"<<"\t total events = "<<Events[k]<<endl;
	    }
	...
	if(BGS==1)
          {
            HistTempSingleHadron[k]->Write();
            HistTempSingleHadronPositive[k]->Write();
            HistTempSingleHadronNegative[k]->Write();
          }
        else
          {
            HistTempSingleHadron[k]->Write();
    ...
    for (int i = 0; i < NpTHardBin; i++)
    { 
      delete HistTempSingleHadron[i]; delete HistTempSingleHadronPositive[i]; delete HistTempSingleHadronNegative[i];
      HistTempSingleHadron[i]=nullptr; HistTempSingleHadronPositive[i]= nullptr; HistTempSingleHadronNegative[i]=nullptr;
    }
