...
// [1] InputDataFileDIR [2] OutputDataFileName  [3]=CMS or Atlas or Alice or STAR, [4]= 200, 2760 or 5020, 7000 [5]=JetRadius, [6]=EtaJetMin, [7]=EtaJetMax    [8] EtaTrackMax [9] pTTrackMin [10] EtaCutFlag =eta or y [11] ChargedFlag 0 or 1 (all or charged)  [12]=JetscapeORPythia, [13] HadronORParton [14] BGS ( 0 or 1 pp, pbpb)  [15] Centrality [16] Species= pp or PbPb pPb    [17] EtaRangeType (2 for two sided, 1 for one sided)
   ...
   sprintf(OutputDataFileName,"%s_R0p%s","JetCrossSection_5020GeV_Alice_Jetscape_Cent_30-50_ChargedHadron_BGS1",argv[1]); //,argv[2]);
   sprintf(JetscapeORPythia,"%s","Jetscape");//,argv[12]);
   sprintf(HadronORParton,"%s","Hadron");//,argv[13]);
   
   string ExpName="Alice";//string(argv[3]);
   int Ecm = 5020;//atoi(argv[4]);
   double JetRadius = atof(argv[1])/10.0;//atof(argv[5])/1.0;                                
   double EtaJetMin = 0.0;//atof(argv[6])/1.0;
   double EtaJetMax= 0.9 - JetRadius;//atof(argv[7])/1.0;                                                        
   double EtaTrackMax=4.0;//atof(argv[8])/1.0;
  ...
  if(  Ecm == 5020 &&  (PPorPbPb=="ppPbPb" || PPorPbPb=="PbPb") && ExpName=="Alice" && Centrality=="30-50" && BGS>=0)
    {
      double Bin[9]  ={30, 40, 50, 60, 70, 80, 100, 120, 140};
      NpTJetBin = 9-1;     JetpTMin = 20;   AliceTrackpT=5.0;
      JetpTBin = new double[NpTJetBin+1];
      for(int i=0; i<NpTJetBin+1; i++)
        {
          JetpTBin[i] = Bin[i];
...
// Histograms.
  ...
  sprintf(VarFileName,"/wsu/home/fy/fy41/fy4125/RUN/Analysis/JetFiles/High0-30V2Cuts%s.txt",OutputDataFileName);
  foutput.open(VarFileName,ios::out);
  sprintf(VarFileName,"/wsu/home/fy/fy41/fy4125/RUN/Analysis/JetFiles/High0-30InPlaneV2Cuts%s.txt",OutputDataFileName);
  foutputInPlane.open(VarFileName,ios::out);
  sprintf(VarFileName,"/wsu/home/fy/fy41/fy4125/RUN/Analysis/JetFiles/High0-30OutPlaneV2Cuts%s.txt",OutputDataFileName);
  foutputOutPlane.open(VarFileName,ios::out);
  sprintf(VarFileName,"/wsu/home/fy/fy41/fy4125/RUN/Analysis/JetFiles/Low0-30V2Cuts%s.txt",OutputDataFileName);
  foutput2.open(VarFileName,ios::out);
  sprintf(VarFileName,"/wsu/home/fy/fy41/fy4125/RUN/Analysis/JetFiles/Low0-30InPlaneV2Cuts%s.txt",OutputDataFileName);
  foutput2InPlane.open(VarFileName,ios::out);
  sprintf(VarFileName,"/wsu/home/fy/fy41/fy4125/RUN/Analysis/JetFiles/Low0-30OutPlaneV2Cuts%s.txt",OutputDataFileName);
  foutput2OutPlane.open(VarFileName,ios::out);

  sprintf(VarFileName,"/wsu/home/fy/fy41/fy4125/RUN/Analysis/JetFiles/Full0-100V2Cuts%s.txt",OutputDataFileName);
  foutputFull.open(VarFileName,ios::out);
  sprintf(VarFileName,"/wsu/home/fy/fy41/fy4125/RUN/Analysis/JetFiles/Full0-100InPlaneV2Cuts%s.txt",OutputDataFileName);
  foutputFullInPlane.open(VarFileName,ios::out);
  sprintf(VarFileName,"/wsu/home/fy/fy41/fy4125/RUN/Analysis/JetFiles/Full0-100OutPlaneV2Cuts%s.txt",OutputDataFileName);
	 ...
	 if( MyString.compare(EventLabel)==0 && fjInputs.size()>0 )
	   {
		 ...
		 if(ExpName=="Alice" || ExpName=="STAR" || ExpName=="sPHENIX" )
		   {
			 ...
			 for (int j = 0; j < int(constituents.size()); j++)
			   {
			     if( ExpName=="Alice" && pythia.particleData.charge(constituents[j].user_index())!=0 && constituents[j].pt() > AliceTrackpT )
			       {
				 GoodJetStatus[i]=1; j=int(constituents.size());
			       }
			     if( (ExpName=="STAR" || ExpName=="sPHENIX") && constituents[j].pt() > STARTrackpT )
                               {
                                 GoodJetStatus[i]=1; j=int(constituents.size());
                               }
			   }
		 ...
		 if(BGS==1)
		     ...
		     if (nListJets)
                       { cout<<"PSI2 = "<<PSI2 <<", "<<PSI2*180.0/PI<<endl; 
		 ...
		 for (int i = 0; i < pFast; ++i)
		   {
			 ...
			 if( EtaRangeType==2 && EtaJetMin <= fabs(JetEta) && fabs(JetEta) < EtaJetMax)                       
			   {			     
			     PhiJet = SortedJets[i].phi()*180/PI;
				 ...
				 if( DeltaPsi < 30 ){HistTempInPlane->Fill(SortedJets[i].perp() );}
				 if( DeltaPsi > 60 ){HistTempOutPlane->Fill(SortedJets[i].perp() );}
			       }
			     //Low-v2 events
			     if(q2Vector < Q2VectorCuts[1] )
			       { HistTemp2->Fill( SortedJets[i].perp() );
				 if( DeltaPsi < 30 ){HistTemp2InPlane->Fill(SortedJets[i].perp() );}
                                 if( DeltaPsi > 60 ){HistTemp2OutPlane->Fill(SortedJets[i].perp() );}
			       }
			     //All v2 events
			     HistTempFull->Fill(SortedJets[i].perp());
			     if( DeltaPsi < 30 ){HistTempFullInPlane->Fill(SortedJets[i].perp() );}
			     if( DeltaPsi > 60 ){HistTempFullOutPlane->Fill(SortedJets[i].perp() );}
			   }			                            			                          
